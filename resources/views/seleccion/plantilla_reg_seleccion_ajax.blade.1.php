										...
										@foreach($jugadores as $usuario)
											...
											@elseif($dato->disciplina->categoria ==0)
												<tr>
													...
													{{--  <td>{{ $usuario->inscripcion->admin_club->club->nombre_club }}</td>  --}}
													<td>
														{!! Form::checkbox('id_[]',$usuario->jugador->jugador_clubs->where('id_club',$usuario->inscripcion->admin_club->id_club)->first()->id_jug_club, false, ['class'=>'check_us']) !!}
													</td>
												</tr>
											@endif
						...
						<div class="form-group col-xl-6 table-responsive-md">
							<table class="table table-striped table-bordered">
								...
								<tbody>
									...
									@endforeach
										<tr>
											<td colspan="5" class="text-center">{!! Form::submit('Deshabiltar', ['class'=>'btn btn-secondary']) !!}</td>
										</tr>
								</tbody>
								{!! Form::close() !!}
							</table>
